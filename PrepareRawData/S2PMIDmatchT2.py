# 2 号进程
# nohup python -u S2PMIDmatchT2.py > S2PMIDmatchT2.log 2>&1 &
    ...
    with jsonlines.open('/home/dell/kd_paper_data/data/SemanticScholar-20220913/full(v20220913)/papers/paperP%d.jsonl'%i, mode='r') as reader:
        for item in reader:
            if item['externalids']['PubMed'] != None and item['externalids']['PubMed'] in PID_set:
                pid_s2id2[item['externalids']['PubMed']] = item['corpusid']
print('# matched-t2',len(pid_s2id2))
pk.dump(pid_s2id2,open('../../data/processedData/pid_s2idT2.pkl','wb'))